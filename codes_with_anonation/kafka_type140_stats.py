...
"""
===============================================================================
【脚本用途】kafka_type140_stats.py — Kafka type=140 消息摸底统计
===============================================================================
从 topic 最早 offset 扫描至启动时 end offset（不提交 group offset），统计：
    - type=140 消息数、唯一 assetId、重复消息
    - vectorItem / vectorList 各 type 的向量校验状态（valid/length_invalid 等）
    - 可选保存原始 payload 样本到 txt

依赖：kafka-python、PyYAML

典型用法：
    python kafka_type140_stats.py -c config.yml
    python kafka_type140_stats.py -c config.yml --max-scan 50000 --save-samples log/samples.txt
    python kafka_type140_stats.py -c config.yml --max-matched 100 --top-n 30
===============================================================================
"""
...
# =============================================================================
# 【load_config】 读取 YAML 配置并合并 DEFAULT_CONFIG
# =============================================================================
...
# =============================================================================
# 【validate_config】 配置强校验（bootstrap_servers/topic/group_id/thrift_hosts 必填）
# =============================================================================
...
# =============================================================================
# 【configure_logging】 配置 logging 格式与级别
# =============================================================================
...
# =============================================================================
# 【parse_bootstrap_servers】 解析 Kafka broker 列表（逗号字符串或 list）
# =============================================================================
...
# =============================================================================
# 【parse_api_version】 解析 Kafka api_version，如 "2.0.0" → (2,0,0)
# =============================================================================
...
# =============================================================================
# 【normalize_json_strings】 递归解析 JSON 字符串套娃（POMS 双重编码）
# =============================================================================
...
# =============================================================================
# 【flatten_fields】 嵌套结构拍平为 path→value，供 find_field 兜底查找
# =============================================================================
...
# =============================================================================
# 【find_field】 带别名与嵌套路径的字段查找（assetId/vector 等）
# =============================================================================
...
# =============================================================================
# 【find_mapping_by_key】 在嵌套结构中按 key 名查找 dict 子对象
# =============================================================================
...
# =============================================================================
# 【find_list_by_key】 在嵌套结构中按 key 名查找 list
# =============================================================================
...
# =============================================================================
# 【find_vector_item】 查找 vectorItem 对象
# =============================================================================
...
# =============================================================================
# 【find_vector_list】 查找 vectorList 数组
# =============================================================================
...
# =============================================================================
# 【find_vector_item_field】 优先从 vectorItem 取字段，否则 find_field
# =============================================================================
...
# =============================================================================
# 【validate_vector_string】 base64 向量校验，返回 status/bytes/dimension 统计结构
# =============================================================================
...
# =============================================================================
# 【increment_nested_counter】 嵌套 Counter 累加（vectorList 分 type 统计用）
# =============================================================================
...
# =============================================================================
# 【update_vector_stats】 更新 vectorItem 相关统计计数
# =============================================================================
...
# =============================================================================
# 【update_vector_list_stats】 更新 vectorList 各 type 的向量校验统计
# =============================================================================
...
# =============================================================================
# 【print_progress】 打印扫描进度（已扫/匹配/唯一 assetId/向量 valid 数等）
# =============================================================================
...
# =============================================================================
# 【wait_topic_partitions】 轮询等待 Kafka topic 分区元数据就绪
# =============================================================================
...
# =============================================================================
# 【scan_topic】 从 earliest 扫描 topic 至启动时 end offset，统计 type=140 消息
# ----------------------------------------------------------------------------
# 不提交 consumer group offset；支持 --max-scan / --save-samples
# =============================================================================
        ...
        while not stop["requested"]:
            ...
            for records in records_by_partition.values():
                for message in records:
                    ...
                    except Exception as exc:  # 解析失败计入 parse_errors，继续扫描
                    ...
                    # 只统计 type=140（或 --filter-type 指定类型）
