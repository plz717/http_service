...
"""
===============================================================================
【脚本用途】calculate_content_status.py — 挂件/点播/二台内容下线状态同步
===============================================================================
架构：
    Kafka（挂件 topic / 点播 topic）
        │
        ▼  版本防倒灌（Redis pendant_version:/ondemand_version:）
        ▼  业务规则判定是否下线（赛事/赛季/测试/roomStatus 等）
        ▼  下线时同步：HBase(all/latest/cp) + 搜推 ES + Redis HTTP 接口

类：CalculateContentStatus
    - GetOnePendantStatus：单条挂件/二台是否应下线
    - ConsumerPendantKafka：消费挂件消息（activityType 0/1/4）
    - ConsumerOndemandKafka：消费点播消息（type 208/217, operation=2 下线）

依赖：TimeUtils、RedisManager、EsManager、HappyBaseClient、kafka_client、
      persistent_module_image、elasticsearch、requests

典型用法（由 shell 脚本传入 param_spec JSON）：
    python calculate_content_status.py --func_type pendant --param_spec /path/to/spec.json --target_dir /data
    python calculate_content_status.py --func_type ondemand --param_spec /path/to/spec.json --target_dir /data
    python calculate_content_status.py --func_type refresh --param_spec /path/to/spec.json
===============================================================================
"""
...
# =============================================================================
# 【CalculateContentStatus】 内容上下线状态计算与多存储同步
# ----------------------------------------------------------------------------
# 消费挂件/点播 Kafka，判定下线条件后更新 HBase（all/latest/cp）、ES、Redis
# 依赖：HappyBaseClient / EsManager / RedisManager / PersistentModule
# =============================================================================
class CalculateContentStatus:
# --- 初始化 CalculateContentStatus ---
...
# --- 判定单条挂件/二台是否应下线（赛事/赛季/测试/状态等） ---
# =============================================================================
# 【GetOnePendantStatus】 判定挂件/二台是否在线
# ----------------------------------------------------------------------------
# activityType 0/1：赛事赛季、测试标题、mgdbStatus 等；4：二台 roomStatus/broadcastStatus
# 返回 1=在线，0=应下线，异常返回 None
# =============================================================================
...
# --- 消费挂件 Kafka：版本防倒灌 → 下线则同步 HBase/ES/Redis ---
# =============================================================================
# 【ConsumerPendantKafka】 挂件 Kafka 消费主循环
# ----------------------------------------------------------------------------
# 过滤 channel → 版本防倒灌 → GetOnePendantStatus → 下线则更新 HBase/ES/Redis
# =============================================================================
        ...
        for msg in self.consumer.consume_data():
            try:
                ...
                if obj["activityType"] in [0, 1]:
                    ...
                    if rvalue:
                        # 版本防倒灌：Redis 中已有更高 version 则跳过
                        if int(obj["version"]) < int(rvalue):
                            continue
                    status = self.GetOnePendantStatus(data)  # 1=在线 0=应下线
...
# --- 消费点播 Kafka：operation=2 下线，同步多存储 ---
# =============================================================================
# 【ConsumerOndemandKafka】 点播 Kafka 消费主循环
# ----------------------------------------------------------------------------
# type 208/217 + 视讯产品线过滤；operation=2 触发点播下线多存储同步
# =============================================================================
...
# --- 清理并重建 video_search_recomm_content_offline 离线 zset ---
